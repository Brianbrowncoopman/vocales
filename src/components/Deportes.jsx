...
import CubosDeportes from "./CubosDeportes"; 
import ImagenPalabra from "./ImagenPalabra";
import { useNavigate } from "react-router-dom";
import DeportesPorLetras from "./DeportesPorLetras";
...
const Deportes = () => {
  const todasLasPalabras = DeportesPorLetras;
  ...
  const deporteActual = palabraActual.nombre ? palabraActual.nombre.toLowerCase() : "";

  const [deportesEnCubos, setDeportesEnCubos] = useState([]);
  ...
  useEffect(() => {
    if (deporteActual) {
      const opciones = getRandomOpciones(todasLasPalabras, deporteActual, 4);
      setDeportesEnCubos(opciones);
    }
  }, [deporteActual, todasLasPalabras]);
  ...
  const verificarSeleccion = (v) => {
    ...
    if (String(v).toLowerCase() === deporteActual) {
  ...
  return (
    <div className="vocales-container" style={fondoStyle}>
      ...
      <div className="cubos-container">
        {deportesEnCubos.map((deporte, idx) => (
          <CubosDeportes
            key={deporte}
            fruta={deporte}
            color={coloresCubo[idx % coloresCubo.length]}
            seleccionado={seleccion === deporte}
            onSeleccionar={(deporte) => {
              if (iniciado && !feedbackSound) verificarSeleccion(deporte);
            }}
          />
        ))}
      </div>
