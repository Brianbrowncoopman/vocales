...
import CubosFrutas from "./CubosFrutas"; 
...
const FrutasVerduras = () => {
  const todasLasPalabras = FrutasPorLetras;
  ...
  const frutaActual = palabraActual.nombre ? palabraActual.nombre.toLowerCase() : "";

  const [frutasEnCubos, setFrutasEnCubos] = useState([]);
  ...
  useEffect(() => {
    if (frutaActual) {
      const opciones = getRandomOpciones(todasLasPalabras, frutaActual, 4);
      setFrutasEnCubos(opciones);
    }
  }, [frutaActual, todasLasPalabras]);
  ...
  const verificarSeleccion = (v) => {
    ...
    if (String(v).toLowerCase() === frutaActual) {
  ...
  return (
    <div className="vocales-container" style={fondoStyle}>
      ...
      <div className="cubos-container">
        {frutasEnCubos.map((fruta, idx) => (
          <CubosFrutas
            key={fruta}
            fruta={fruta}
            color={coloresCubo[idx % coloresCubo.length]}
            seleccionado={seleccion === fruta}
            onSeleccionar={(fruta) => {
              if (iniciado && !feedbackSound) verificarSeleccion(fruta);
            }}
          />
        ))}
      </div>
