...
import CubosAnimales from "./CubosAnimales"; 
import ImagenPalabra from "./ImagenPalabra";
import AnimalesPorLetra from "./AnimalesPorLetra";
...
const Animales = () => {
  const todasLasPalabras = AnimalesPorLetra;
  ...
  const animalActual = palabraActual.nombre ? palabraActual.nombre.toLowerCase() : "";

  const [animalesEnCubos, setAnimalesEnCubos] = useState([]);
  ...
  useEffect(() => {
    if (animalActual) {
      const opciones = getRandomOpciones(todasLasPalabras, animalActual, 4);
      setAnimalesEnCubos(opciones);
    }
  }, [animalActual, todasLasPalabras]);
  ...
  const verificarSeleccion = (v) => {
    ...
    if (String(v).toLowerCase() === animalActual) {
  ...
  return (
    <div className="vocales-container" style={fondoStyle}>
      ...
      <div className="cubos-container">
        {animalesEnCubos.map((animal, idx) => (
          <CubosAnimales
            key={animal}
            fruta={animal}
            color={coloresCubo[idx % coloresCubo.length]}
            seleccionado={seleccion === animal}
            onSeleccionar={(animal) => {
              if (iniciado && !feedbackSound) verificarSeleccion(animal);
            }}
          />
        ))}
      </div>
