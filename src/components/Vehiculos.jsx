...
import CubosVehiculos from "./CubosVehiculos"; 
import ImagenPalabra from "./ImagenPalabra";
import VehiculosPorLetra from "./VehiculosPorLetra";
...
const Vehiculos = () => {
  const todasLasPalabras = VehiculosPorLetra;
  ...
  const vehiculoActual = palabraActual.nombre ? palabraActual.nombre.toLowerCase() : "";

  const [vehiculosEnCubos, setVehiculosEnCubos] = useState([]);
  ...
  useEffect(() => {
    if (vehiculoActual) {
      const opciones = getRandomOpciones(todasLasPalabras, vehiculoActual, 4);
      setVehiculosEnCubos(opciones);
    }
  }, [vehiculoActual, todasLasPalabras]);
  ...
  const verificarSeleccion = (v) => {
    ...
    if (String(v).toLowerCase() === vehiculoActual) {
  ...
  return (
    <div className="vocales-container" style={fondoStyle}>
      ...
      <div className="cubos-container">
        {vehiculosEnCubos.map((vehiculo, idx) => (
          <CubosVehiculos
            key={vehiculo}
            vehiculo={vehiculo}
            color={coloresCubo[idx % coloresCubo.length]}
            seleccionado={seleccion === vehiculo}
            onSeleccionar={(vehiculo) => {
              if (iniciado && !feedbackSound) verificarSeleccion(vehiculo);
            }}
          />
        ))}
      </div>
